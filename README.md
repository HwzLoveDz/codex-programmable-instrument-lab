...
已有完整或受限实机流程覆盖 **SIGLENT SDS3104X HD**、**tinySA Ultra+ ZS407**、**LiteVNA 64 ZN-406** 和 **FLUKE 8845A**。另完成一轮 **MDP-M01 + P906 + L1060 + FLUKE 8845A** 低功率功能联调。MDP-M01 使用 MINIWARE 专有 USB 二进制协议，不是 SCPI；受限适配器已实机完成 5 V 电源与 0.1 A CC 负载闭环，并由 FLUKE 独立测量电压。这是有限功能验证，不代表额定范围或精度验收。不同型号不共用未经验证的命令或传输假设。

实板联合调试时，先由硬件工程技能根据需求、板版本和额定值确定测点、参考地、接线风险、激励上限及判据，再由本技能选择满足精度／带宽／安全要求且已验证可控的仪器执行，最后以独立回读和原始记录交回硬件技能分析。缺少物理接线或关键安全条件时由操作者确认，不从软件状态猜测。
...
- MDP-M01 持久 USB 会话、状态读取、受限 P906／L1060 CC 设定与回读、逐条确认的输出控制，以及 P906+L1060 的 5 V／0.1 A CC 低功率实机闭环；
...
以上通道号和数值只是示例：必须先按本次 `status` 回读辨认通道，确认真实接线、极性、负载和风险上限，再逐步操作。`arm` 要求所有在线输出均为 OFF；设置值也只能在相应输出 OFF 时更改。每个状态变化后等新的状态帧核对；P906 ON 后应先用独立仪表确认端子稳定达到目标，再启用 L1060。M01 输出位与模拟电压字段可能不同步，端子是否达到目标或归零须独立测量。若输出开关回读超时／不匹配，不可重发；先让操作者检查实物状态。`end` 只读最后状态并关闭 USB 连接，不会自动关闭任何输出；结束实验前应显式关闭并确认相关输出为 OFF，但 OFF 不等于端子零伏。适配器已完成一次 5 V P906／0.1 A L1060 CC 低功率闭环；其他模式、量程和精度仍需各自验证。详细命令字段和证据边界见 [MDP 联调参考](references/mdp-m01-p906-l1060.md)。

2026-09-23 曾用临时实验程序完成 5 V、100 mA CC 轻载探索，准确度判据未定义。2026-09-24 新适配器闭环中，FLUKE 空载均值 5.005994 V，接入 0.1 A CC 后为 5.001688 V，变化 −4.306 mV；M01 后续状态帧报负载 0.099 A、电源 0.107 A。两路输出均获 OFF 回读，关机约 20 秒后端子降至 3.11 mV。测试验证有限低功率控制与状态闭环，没有判定电源准确度；M01 的部分模拟电压字段与独立表读数存在差异，详情见 [MDP 联调参考](references/mdp-m01-p906-l1060.md)。
